...
#define HWY_TARGET_INCLUDE "third_party/libjxltiny/encoder/enc_adaptive_quantization.cc"
...
#include "third_party/libjxltiny/encoder/fast_math-inl.h"
...
void ComputeAdaptiveQuantFieldTile(const Image3F& xyb, const Rect& rect,
  ...
  pre_erosion->ShrinkTo((x1 - x0) / 4, (y_end - y_start) / 4);
  ...
  for (size_t y = y_start; y < y_end; ++y) {
    ...
    for (; x + 1 + Lanes(df) < x1; x += Lanes(df)) {
      ...
      auto base_x = Mul(quarter, Add(Add(in_x_r, in_x_l), Add(in_x_t, in_x_b)));
    ...
    if (y % 4 == 3) {
      float* row_dout = pre_erosion->Row((y - y_start) / 4);
